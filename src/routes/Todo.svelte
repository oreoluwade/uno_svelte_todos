<script lang="ts">
	import { todosStore, type Todo } from '$lib/stores/todo-store';
	...
	const handleEdit = (event: KeyboardEvent, id: string) => {
		...
		switch (pressedKey) {
			...
			case 'Enter':
				todosStore.editTodo(id, newTodoText);
				targetElement.blur();
				break;
	...
	<div class="todo-item">
		<div>
			<input
				...
				on:change={() => {
					todosStore.markTodoAsComplete(todo.id);
				}}
			/>
			<label aria-label="Check todo" class="todo-check" for="todo" />
		</div>
		<!-- svelte-ignore a11y-no-static-element-interactions -->
		<span class="todo-text" class:completed={todo.completed} on:dblclick={toggleEdit}
			>{todo.title}</span
		>
		<button
